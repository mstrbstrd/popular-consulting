...
describe("shared Aetheris site styling", () => {
  ...
  test("loads the shared layer after the legacy global stylesheet", () => {
    ...
    const aetherisImport = indexSource.indexOf("import './aetheris-site.css';");
  ...
  test("covers every shared shell and content surface", () => {
    [
      ".nav-pill",
      ".section-dot",
      ".bio-card",
  ...
  test("does not reintroduce the standalone legacy purple treatment", () => {
    [
      ...
      "rgb(173, 118, 215)",
      "#b989dd",
    ].forEach((legacyValue) => expect(normalizedCss).not.toContain(legacyValue));
  ...
  test("loads the Technical-Humanist pair on generated shared routes", () => {
    ...
    expect(routeGenerator).toContain("const ENGINEERING_FONTS_HREF = IMMERSIVE_FONTS_HREF;");
