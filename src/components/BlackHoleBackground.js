...
// DISK_OUTER = 12 in the shader, so zoom ~14 puts camera just outside the disk edge.
const SECTION_ZOOMS = [
  14,  // 0 Hero     — extreme close-up, event horizon dominates
  28,  // 1 Bio      — mid-range, full disk visible
  44,  // 2 Services — wide, disk is a bright ring in space
  62,  // 3 Contact  — distant, BH small against star field
  22,  // 4 Orb      — not visible (OrbSection covers it)
  18,  // 5 Game     — not visible (Game canvas covers it)
];
...
// ─── Fragment shader (verbatim copy from BlackHoleCanvas, minus u_lightMode) ─
...
const BlackHoleBackground = ({ activeSection = 0 }) => {
  ...
  useEffect(() => {
    window.__bhRevealStart = () => { currentZoomRef.current = 80; };
    return () => { window.__bhRevealStart = null; };
  }, []);
  ...
  useEffect(() => {
    ...
    const compileShader = (type, src) => {
      ...
      if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
        console.error('BlackHoleBackground shader error:', gl.getShaderInfoLog(s));
        return null;
      }
      return s;
    };
    ...
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
      console.error('BlackHoleBackground link error:', gl.getProgramInfoLog(prog));
      return;
    }
    gl.useProgram(prog);
