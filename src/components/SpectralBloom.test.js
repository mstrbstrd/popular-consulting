...
describe("SpectralBloom", () => {
  ...
  test("renders a decorative canvas that degrades safely without WebGL", () => {
    ...
    const { container, unmount } = render(<SpectralBloom />);
  ...
  test("keeps the shader inside the kit's discipline", () => {
    ...
    const source = fs.readFileSync(path.join(__dirname, "SpectralBloom.js"), "utf8");
    ...
    // Scroll listener hygiene: registered passively, removed on cleanup.
