// Canonical original black-hole shaders. Preserve the geodesic fragment shader.

export const BLACK_HOLE_VERTEX_SHADER = `#version 300 es
in vec2 a_pos;
out vec2 vUv;
uniform vec2 u_res;
uniform vec4 u_tile_rect;
void main(){
  vec2 localUv = a_pos * 0.5 + 0.5;
  vUv = (u_tile_rect.xy + localUv * u_tile_rect.zw) / u_res;
...
export const createBlackHoleFragmentShader = () =>
  BLACK_HOLE_FRAGMENT_SHADER;

export const PRESENT_VERTEX_SHADER = `#version 300 es
in vec2 a_pos;
out vec2 v_uv;
void main() {
  v_uv = a_pos * 0.5 + 0.5;
